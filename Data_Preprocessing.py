...
class Mydata(Dataset):
    def __init__(self, S_path, T_path, train_ratio=0.9, test_ratio=0.1, train=None, preprocessed=True):
        ...
        if preprocessed == False:
            ...
            ### 将代码转换为序号
            print("Mapping users and items...")
            S_df["User"] = S_df["User"].map(dict_users)
            S_df["Item"] = S_df["Item"].map(dict_S_items)
            T_df["User"] = T_df["User"].map(dict_users)
            T_df["Item"] = T_df["Item"].map(dict_T_items)
            # 样例
            # Item    User    Rating
            # 22      32          5
            # print(S_df.head())
            ...
            self.S_data[torch.tensor(S_df["User"].values, dtype=torch.long), 
                        torch.tensor(S_df["Item"].values, dtype=torch.long)] = torch.tensor(S_df["Rating"].values, dtype=torch.float32)

            self.T_data[torch.tensor(T_df["User"].values, dtype=torch.long), 
                        torch.tensor(T_df["Item"].values, dtype=torch.long)] = torch.tensor(T_df["Rating"].values, dtype=torch.float32)
...
if __name__ == "__main__":
    data = Mydata(r'./Data/Books.csv',
                  r'./Data/Movies_and_TV.csv', train=True, preprocessed=False)
