...
def run_sla(S_data, T_data, train_indices, test_indices, epochs):
    ...
    def compute_spectral_embeddings(R, k=300):
        A = (R > 0).astype(float)
        ...
        if not isinstance(A, coo_matrix): A = coo_matrix(A)
        M = D_u_inv @ A @ D_i_inv
        try:
            u, s, vt = svds(M, k=k, maxiter=100000)
        except Exception:
            try: u, s, vt = svds(M, k=k//2, maxiter=200000)
            except Exception: u, s, vt = svds(M, k=50, maxiter=500000)
        idx = np.argsort(s)[::-1]
        return u[:, idx], vt[idx, :].T
